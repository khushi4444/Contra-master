...
class Game:
    ...
    def update(self):
        ...
        h1 = pygame.sprite.groupcollide(self.bullets, self.snipers, True, True)
        if h1:
            for k in h1:
                self.deathAnim(k)
        h1 = pygame.sprite.groupcollide(self.bullets, self.soldiers, True, True)
        if h1:
            for k in h1:
                self.deathAnim(k)
        h1 = pygame.sprite.groupcollide(self.bullets, self.tanks, True, True)
        if h1:
            for k in h1:
                self.deathAnim(k)

        # Check player colliding with any bullet
        hits = pygame.sprite.spritecollide(p, self.enemy_bullets, True)
        if hits:
            p.health -= 1
            hit_sound.play()
            if p.health == 0:
        ...
        # or enemy
        h1 = pygame.sprite.spritecollide(p, self.snipers, False)
        if h1:
            p.die()
            self.deathAnim(p)
            self.playing = False
        h1 = pygame.sprite.spritecollide(p, self.soldiers, False)
        if h1:
            p.die()
            self.deathAnim(p)
            self.playing = False
        h1 = pygame.sprite.spritecollide(p, self.tanks, False)
        if h1:
            p.die()
            self.deathAnim(p)
            self.playing = False
        ...
        hits = pygame.sprite.spritecollide(p, self.grounds, False)
        ...
        for e in self.soldiers:
            hits = pygame.sprite.spritecollide(e, self.grounds, False)
            if hits:
                e.pos.y = hits[0].defaulty
                e.vel.y = 0

        # Powerup events
        hits = pygame.sprite.spritecollide(p, self.powerups, False)
        if hits:
            powerup = hits[0]
